...
**Common swaps:** Swap `strategy.red-team` for `strategy.inversion` when the goal is finding structural failure modes rather than credible attacks. Swap `guardrail.assumption-audit` for `guardrail.disconfirming-evidence` when the plan is built around a preferred conclusion.
