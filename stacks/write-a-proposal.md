...
2. `frame.define-success-metrics`
3. `frame.dependency-map`
4. `frame.risk-register`
5. `schema.executive-summary`
6. `guardrail.persuasion-audit`
...
**Minimum blocks:** `frame.clarify-task`, `frame.define-success-metrics`, `schema.executive-summary`

**Why this order works:** Task clarity before writing ensures the proposal addresses the actual ask rather than a slightly different version of it. Success-metrics define the evaluation criteria up front so the proposal can be assessed against them. Dependency-map makes the plan credible. Risk-register prevents the proposal from looking naive by acknowledging what could go wrong. Executive-summary leads with the ask and rationale. Persuasion-audit closes by checking whether a skeptical reviewer would be convinced.

**Common swaps:** Swap `guardrail.persuasion-audit` for `rubric.writing-quality` when the proposal is primarily a written document rather than a pitch. Add `lens.incentive-audit` when the proposal needs to be calibrated to the approver's motivations.

**Common failure mode:** Writing the proposal before defining success criteria. Proposals without upfront success criteria optimize for approval rather than for the outcome the proposal claims to pursue.
