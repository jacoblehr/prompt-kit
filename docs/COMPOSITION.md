...
**Too many strategies:** Each strategy adds cognitive load. Two is usually the ceiling.

**Schema too early:** Putting the output format before the task frame turns the prompt into a form. The model fills boxes instead of reasoning.

**Guardrails everywhere:** Uncertainty guardrails on a fully specified task add noise. Guardrails earn their place only when the failure mode is real.

**Mode switching mid-prompt:** Switching from explore to decide within one prompt often collapses exploration prematurely. If you need both, use a sequence: one session for exploration, one for decision.

**Redundant framing:** `frame.task` + `frame.scope` + `frame.success-criteria` all at once is usually overkill. Pick the one that addresses the actual framing gap.
